...
exports.handler = async (event) => {
  ...
  try {
    ...
    console.error('failed to parse limit or nextKey: ' + e.message)
    return {
      statusCode: 400,
      headers: {
...
/**
 *    helper functions defined by fxrc
 */
function parseLimit(event) {
  ...
  if (limit <= 0) {
    return new Error("limit must be positive!")
  }
...
function parsenextKey(event) {
  ...
  const nextKey = JSON.parse(decodeURIComponent(nextKeyStr))
  return nextKey
}
